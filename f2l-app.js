...
function loadStatuses() {
  try { return JSON.parse(localStorage.getItem("f2l-status")) || {}; } catch { return {}; }
}

function saveStatuses(statuses) {
  localStorage.setItem("f2l-status", JSON.stringify(statuses));
}
...
function buildSidebar() {
  if (typeof F2L_DATA === "undefined") {
    console.error("F2L_DATA not found");
    return;
  }

  const statuses = loadStatuses();
  let html = "";
  F2L_DATA.forEach((group) => {
    html += `<div class="nav-group">`;
    html += `<div class="nav-group-title">${group.category}</div>`;
    group.cases.forEach((c) => {
      const badge = c.name.replace("F2L ", "");
...
async function loadCase(name) {
  ...
  // Info card
  $("info-recognition").textContent = data.recognition;
...
async function createPlayer(data) {
  ...
  player = new TwistyPlayer({
    puzzle: "3x3x3",
    alg: data.algorithm,
    experimentalSetupAlg: data.setup_algorithm,
    visualization: "3D",
    controlPanel: "bottom",
    background: "none",
...
const CAT_ICONS = {
  "Basic Inserts": { cls: "basic", diagram: ["slot","edge","corner","slot"] },
  "Corner In Slot": { cls: "corner-slot", diagram: ["empty","edge","corner","slot"] },
  "Pieces In Slot": { cls: "pieces-slot", diagram: ["corner","ghost","ghost","edge"] },
  "Edge In Slot": { cls: "edge-slot", diagram: ["corner","ghost","slot","edge"] },
  "Connected Pairs": { cls: "connected", diagram: ["corner","edge","empty","empty"] },
  "Disconnected Pairs": { cls: "disconnected", diagram: ["corner","empty","empty","edge"] },
};

function buildCategoryGrid() {
  const grid = $("f2l-cat-grid");
  if (!grid || typeof F2L_DATA === "undefined") return;

  grid.innerHTML = F2L_DATA.map((group) => {
    const icon = CAT_ICONS[group.category] || { cls: "basic", diagram: ["corner","edge","empty","empty"] };
    const count = group.cases.length;
    return `
      <div class="f2l-cat-card" data-cat="${group.category}">
        <div class="f2l-cat-icon ${icon.cls}">
          <div class="pair-diagram">
            ${icon.diagram.map(c => `<div class="pd ${c}"></div>`).join("")}
          </div>
        </div>
        <h3>${group.category}</h3>
        <span class="f2l-cat-count">${count} case${count !== 1 ? "s" : ""}</span>
      </div>
    `;
  }).join("");

  grid.addEventListener("click", (e) => {
    const card = e.target.closest(".f2l-cat-card");
    if (!card) return;
    const cat = card.dataset.cat;
    // Find first case in this category
    const group = F2L_DATA.find(g => g.category === cat);
    if (group && group.cases.length > 0) {
      loadCase(group.cases[0].name);
      // Open sidebar on mobile to show the group
