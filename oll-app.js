...
function loadStatuses() {
  try { return JSON.parse(localStorage.getItem("oll-status")) || {}; } catch { return {}; }
}

function saveStatuses(statuses) {
  localStorage.setItem("oll-status", JSON.stringify(statuses));
}
...
function buildSidebar() {
  if (typeof OLL_DATA === "undefined") {
    console.error("OLL_DATA not found");
    return;
  }

  const statuses = loadStatuses();
  let html = "";
  OLL_DATA.forEach((group) => {
    html += `<div class="nav-group">`;
    html += `<div class="nav-group-title">${group.category}</div>`;
    group.cases.forEach((c) => {
      const badge = c.name.replace("OLL ", "");
...
function render2DDiagram(diagramData) {
  ...
  if (!container) return;
  if (!diagramData) { container.innerHTML = ""; return; }

  const { top_face, top_edge, right_edge, bottom_edge, left_edge } = diagramData;
  const cells = [];

  // Row 0
  cells.push("empty", top_edge[0] ? "oll-side" : "empty", top_edge[1] ? "oll-side" : "empty", top_edge[2] ? "oll-side" : "empty", "empty");

  // Rows 1-3
  for (let i = 0; i < 3; i++) {
    cells.push(left_edge[i] ? "oll-side" : "empty");
    for (let j = 0; j < 3; j++) {
      cells.push(top_face[i][j] ? "yellow" : "oll-blank");
    }
    cells.push(right_edge[i] ? "oll-side" : "empty");
  }

  // Row 4
  cells.push("empty", bottom_edge[0] ? "oll-side" : "empty", bottom_edge[1] ? "oll-side" : "empty", bottom_edge[2] ? "oll-side" : "empty", "empty");
